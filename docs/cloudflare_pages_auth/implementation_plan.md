...
Cloudflare Pagesへのアップロード（デプロイ）は、GitHubとの連携機能を使用するのが最も簡単で確実です。

1. **Cloudflare Dashboardにログイン**: [Cloudflare Dashboard](https://dash.cloudflare.com/) にアクセスします。
2. **Workers & Pages**: 左メニューから `Workers & Pages` > `概要` を選択します。
3. **アプリケーションの作成**: `アプリケーションの作成` ボタンを押し、`Pages` タブを選択して `Gitに接続` をクリックします。
4. **リポジトリの選択**: `bousouninjin/Astrology-i18n` リポジトリを選択します。
5. **ビルド設定**:
    - **フレームワーク プリセット**: `Astro` を選択します。
    - **ビルド コマンド**: `npm run build` (または `astro build`)
    - **ビルド出力ディレクトリ**: `dist`
6. **環境変数**:
    - `NODE_VERSION`: `20` (必要に応じて指定)
    - `APP_SECRET_TOKEN`: (ここで設定するか、デプロイ後に設定画面から追加します)
7. **保存してデプロイ**: 設定を保存すると、初回のビルドとデプロイが開始されます。

以降は、GitHubにコードをプッシュするだけで自動的にCloudflare Pagesにアップロード・更新されます。functionsディレクトリ内のコードも自動的にデプロイされます。
